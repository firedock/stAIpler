...
function getLayerGuidance(kind: LayerType): string {
  const guidance: Record<LayerType, string> = {
    ...
    continuity: 'Records session handoffs under docs/handoffs/ so the next agent can pick up where the last one left off. Each handoff carries YAML frontmatter (title, thread, status, summary) and is generated by the /handoff skill at the end of a working session. Not written by the optimizer directly — treat as runtime context authored per-session.',
  };
